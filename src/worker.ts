...
import { formatErrorReply, sendReply } from "./mailer";
...
async function processJob(job: EmailJob): Promise<void> {
	const retryInfo = job.retryCount
		? ` (retry ${job.retryCount}/${MAX_RETRIES})`
		: "";
...
async function startRetryProcessor(redis: RedisClient): Promise<void> {
	...
	while (true) {
		try {
			...
			if (movedCount > 0) {
				console.log(
					`[Worker] Retry processor moved ${movedCount} job(s) to pending queue`,
				);
			}
		} catch (error) {
