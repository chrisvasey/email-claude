...
async function runGh(projectPath: string, args: string[]): Promise<string> {
	...
	if (exitCode !== 0) {
		const stderr = await new Response(proc.stderr).text();
		throw new Error(
			`gh ${args[0]} failed: ${stderr.trim() || `exit code ${exitCode}`}`,
		);
	}
...
export async function commitAndPush(
	...
	const branchName = await runGit(projectPath, [
		"rev-parse",
		"--abbrev-ref",
		"HEAD",
	]);
...
export async function mergePR(
	...
	await runGh(projectPath, ["pr", "merge", prNumber.toString(), "--merge"]);
}
...
export async function closePR(
	...
	await runGh(projectPath, ["pr", "close", prNumber.toString()]);
}
