...
const mockRedis = {
	zAdd: mock((_key: string, _entry: { score: number; value: string }) =>
		Promise.resolve(1),
	),
	zRangeByScore: mock((_key: string, _min: number, _max: number) =>
		Promise.resolve([] as string[]),
	),
	zRem: mock((_key: string, _value: string) => Promise.resolve(1)),
	rPush: mock((_key: string, _value: string) => Promise.resolve(1)),
	lPush: mock((_key: string, _value: string) => Promise.resolve(1)),
};
...
describe("Worker Retry Logic", () => {
	...
	describe("processRetryQueue", () => {
		...
		it("returns 0 when no jobs are ready", async () => {
			...
			const readyJobs = await mockRedis.zRangeByScore(
				RETRY_QUEUE_NAME,
				0,
				Date.now(),
			);
