...
import type { ClaudeResult, EmailJob } from "../src/mailer.ts";
...
describe("mailer module", () => {
	...
	const createMockResult = (
		overrides: Partial<ClaudeResult> = {},
	): ClaudeResult => ({
	...
	describe("formatSuccessReply", () => {
		test("generates correct text format", async () => {
			const { formatSuccessReply, _resetResendClient } = await import(
				"../src/mailer.ts"
			);
			_resetResendClient();
	...
	describe("formatErrorReply", () => {
		test("includes error message", async () => {
			const { formatErrorReply, _resetResendClient } = await import(
				"../src/mailer.ts"
			);
			_resetResendClient();
	...
	describe("sendReply", () => {
		test("calls Resend with correct params", async () => {
			const { sendReply, _resetResendClient } = await import(
				"../src/mailer.ts"
			);
			_resetResendClient();
	...
	describe("getResend", () => {
		test("throws when RESEND_API_KEY is not set", async () => {
			...
			const { getResend, _resetResendClient } = await import(
				"../src/mailer.ts"
			);
			_resetResendClient();
