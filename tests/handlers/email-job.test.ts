...
mock.module("../../src/mailer", () => ({
	...
	formatBranchNoticeEmail: mock(() =>
		Promise.resolve({ to: "", subject: "", text: "" }),
	),
...
mock.module("../../src/branch-safety", () => ({
	ensureOnDefaultBranch: (...args: unknown[]) =>
		mockEnsureOnDefaultBranch(...args),
}));
...
mock.module("../../src/services/repo", () => ({
	ensureRepo: async (
		...
		if (opts?.fileExists) {
			const exists = await opts.fileExists(
				`${projectsDir}/${project}/.git/config`,
			);
			if (!exists) {
				if (!owner) {
					throw new Error(
				...
				if (opts?.spawn) {
					...
					if (exitCode !== 0) {
						throw new Error(
							`Failed to clone git@github.com:${owner}/${project}.git`,
						);
					}
				}
			}
	...
	getRepoUrl: (project: string, owner: string) =>
		`git@github.com:${owner}/${project}.git`,
	repoExists: async (
		projectPath: string,
		opts?: { fileExists?: (path: string) => Promise<boolean> },
	) => {
		// If opts.fileExists is provided, use it (for repo.test.ts)
		if (opts?.fileExists) {
...
describe("email-job handler", () => {
	...
	beforeEach(() => {
		// Reset all mocks
		mockEnsureOnDefaultBranch = mock(() =>
			Promise.resolve({
	...
	describe("handleEmailJob", () => {
		it("creates branch, runs claude, creates PR with original email, and sends reply", async () => {
			...
			expect(mockBuildFullPrompt).toHaveBeenCalledWith(
				"Add feature request",
				"Add a new feature",
			);
		...
		it("sends error reply on PR comment failure", async () => {
			session.prNumber = 99;
			mockCommentOnPR = mock(() => Promise.reject(new Error("Comment failed")));
