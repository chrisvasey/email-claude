...
describe("git module", () => {
	...
	describe("commitAndPush", () => {
		test("runs add, commit, push in sequence", async () => {
			...
			expect(spawnCalls[2].cmd).toEqual([
				"git",
				"rev-parse",
				"--abbrev-ref",
				"HEAD",
			]);
			expect(spawnCalls[3].cmd).toEqual([
				"git",
				"push",
				"-u",
				"origin",
	...
	describe("createPR", () => {
		...
		test("throws when PR number cannot be parsed", async () => {
			...
			const { createPR } = await import("../src/git.ts");
			await expect(createPR("/test/project", "Title", "Body")).rejects.toThrow(
	...
	describe("runGit error handling", () => {
		test("throws on non-zero exit code", async () => {
			...
			await expect(runGit("/test/project", ["status"])).rejects.toThrow(
	...
	describe("runGh error handling", () => {
		test("throws on non-zero exit code", async () => {
			...
			await expect(runGh("/test/project", ["pr", "list"])).rejects.toThrow(
				"gh pr failed: error: not authenticated",
			);
		});
	});
});
